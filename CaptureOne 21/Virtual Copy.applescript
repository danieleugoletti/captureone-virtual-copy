...
tell the application "Capture One 21"
...
ln \"$filename\" \"$fname-VC$i.$ext\"
ln -s \"$filename\" \"$fname-VC$i.$ext-s\"
echo \"$dirname/$fname-VC$i.$ext\""
	...
	set theBashScript to "#!/bin/bash
fullfilename=" & quoted form of theNewImage & "
rm \"$fullfilename\"
mv \"$fullfilename-s\" \"$fullfilename\""
	
	do shell script theBashScript
